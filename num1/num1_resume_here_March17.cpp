...
int main(){
  ...
  U_pl_profile.resize(size, -1.0);
  Y_pl_profile.resize(size, -1.0);
...
//     Y_pl_profile.push_back(y_pl);
    U_pl_profile[counter] = U_n2;
    Y_pl_profile[counter] = y_pl;
...
//       j = j + 1;
    ...
    fclose(fout);
    std::cout<<"U_plus results file successfully written"<<endl;
